...
sce <- readRDS("_rfiles/_data/clustering_sce_min.rds")
rsec <- readRDS("_rfiles/_data/rsec_min.rds")
```
...
## Trim rsec object
rsec_min <- rsec
rsec_min@assays$data@listData$counts <- 0
rsec_min@assays$data@listData$cpm <- 0
rsec_min@assays$data@listData$logcounts <- 0
rsec_min@colData@listData <- list()
rsec_min@coClustering <- matrix(0)
rsec_min@reducedDims <- SimpleList()
rsec_min@merge_nodeProp <- data.frame()
rsec_min@merge_nodeMerge <- data.frame()
rsec_min@rowRanges <- GRanges()
rsec_min@colData <- DataFrame()

## saveRDS(sce, "_rfiles/_data/clustering_sce_v2.rds", compress = "xz")
## do NOT save the sce file with everything in one SCE!
saveRDS(rsec_min, "_rfiles/_data/rsec_min.rds", compress = 'bz')
saveRDS(sce_min, "_rfiles/_data/clustering_sce_min.rds")
```
