...
tenx_pbmc3k <- readRDS("data/05.integrating-datasets-tenx_pbmc4k.rds")
```
...
plotReducedDim(sce, use_dimred="PCA",
               colour_by="Sample") + ggtitle("Without batch correction")
```

PCA plot after using `removeBatchEffect()` from the `limma` package. 
```{r}
library(limma) 
corrected <- removeBatchEffect(logcounts(sce), batch=sce$Sample)
assay(sce, "corrected_logcounts_limma") <- corrected
rm(corrected)

plotPCASCE(sce, by_exprs_values = "corrected_logcounts_limma", 
        colour_by="Sample")
```
...
sce <- runPCA(sce, exprs_values = "corrected_logcounts_limma")

plotReducedDim(sce, use_dimred="PCA",by_exprs_values = corrected_logcounts_limma, 
               colour_by="Sample") + ggtitle("Without correction")
