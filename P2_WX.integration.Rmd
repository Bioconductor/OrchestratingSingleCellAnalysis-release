...
```{r}
## run MNN with no advanced preprocessing; for comparability, remove cos.norm
tmp <- fastMNN(sce, batch = sce$Sample, cos.norm = FALSE)
...
```{r, fig.cap = 'PCA calculated on the limma batch-corrected gene expression matrix.'}
plotReducedDim(sce, 'MNN_simple', colour_by = 'Sample')
```

```{r, fig.cap = 'UMAP calculated via the PCA derived from limma batch-corrected gene expression matrix.'}
plotReducedDim(sce, 'UMAP_MNN_simple', colour_by = 'Sample')
```
...
```{r, fig.cap = 'Plot of the first two components of the Harmony embedding.'}
plotReducedDim(sce, 'HARMONY', colour_by = 'Sample')
```

```{r, fig.cap = "UMAP calculated on the Harmony embeddings."}
plotReducedDim(sce, 'UMAP_HARMONY', colour_by = 'Sample')
```
...
In this example, we get back `r sum(dec$bio > 0)` genes from the method to use for the integration. For speed, we will limit ourselves to the top 1000 genes.
...
pcs <- multiBatchPCA(sce, batch = sce$Sample,
...
names(reducedDims(sce))[3] <- 'UMAP_MNN'
```
...
```{r, fig.cap = "Plot of first two PCA coordinates. Colour denotes origin of sample."}
plotPCA(sce, use_dimred = 'PCA', colour_by = 'Sample')
```

```{r, fig.cap = "UMAP calculated on the PCA derived from the normalized gene expression matrix."}
plotReducedDim(sce, use_dimred = 'UMAP_PCA', colour_by = 'Sample')
```

```{r, fig.cap = "Plot of first two MNN coordinates. Colour denotes origin of sample.""}
plotReducedDim(sce, 'MNN', colour_by = 'Sample')
```

```{r, fig.cap = "UMAP calculated on the MNN coordinates."}
plotReducedDim(sce, use_dimred = 'UMAP_MNN', colour_by = 'Sample')
```
...
## calc a PCA; save separate to prevent overwriting
tmp <- runPCA(sce,
       ncomponents = 25,
       feature_set = hvg_genes_subset,
...
reducedDim(sce, 'PCA_limma') <- reducedDim(tmp, 'PCA')

## run UMAP; save separate to prevent overwriting
tmp <- runUMAP(sce, use_dimred = 'PCA_limma')
reducedDim(sce, 'UMAP_limma') <- reducedDim(tmp, 'UMAP')
```

```{r, fig.cap = 'PCA calculated on the limma batch-corrected gene expression matrix.'}
plotReducedDim(sce, 'PCA_limma', colour_by = 'Sample')
```

```{r, fig.cap = 'UMAP calculated via the PCA derived from limma batch-corrected gene expression matrix.'}
plotReducedDim(sce, 'UMAP_limma', colour_by = 'Sample')
```
