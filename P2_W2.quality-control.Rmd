...
The function also assumes that cell barcodes with total UMI counts below a certain threshold (`lower=100` by default) correspond to empty droplets, and uses them to estimate the ambient expression profile.
...
The number of Monte Carlo iterations (specified by the `niters` argument in `emptyDrops()`) determines the lower bound for the _p_v
alues [@phipson2010permutation].
...
We can check whether the hypothesis testing procedure holds its size by examining the distribution of $p$-values for low-total barcodes.
...
The use of `isOutlier()` is an effective general strategy for identifying and removing low-quality cells.
However, it involves a number of assumptions about the experiment and biological system, and while these assumptions are mostly reasonable, users should still keep them in mind when interpreting the results.
(Be alert, not alarmed, for those who remember the Australian Government's ~indoctrination~ counter-terror program.)
...
The biggest concern with the assumptions is whether a cell type is lost during the QC procedure.
...
For comparison, we consider our PBMC data set where we remove all libraries with a library size below 500.
