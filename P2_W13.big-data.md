...
## 1 runPCA(tiny_sce, BSPARAM = IrlbaParam()) 557.63ms 557.63ms        0B
## 2 runPCA(tiny_sce, BSPARAM = ExactParam())    1.06s    1.06s        0B
```
...
##   <bch:expr>                                          <bch> <bch:>
## 1 buildSNNGraph(tiny_sce_pca, BNPARAM = AnnoyParam())    5s     5s
## 2 buildSNNGraph(tiny_sce_pca, BNPARAM = KmknnParam()) 3.78s  3.78s
...
## 1 buildSNNGraph(big_sce, BNPARAM = AnnoyParam())    41.5s   41.5s        0B
## 2 buildSNNGraph(big_sce, BNPARAM = KmknnParam())    54.2s   54.2s        0B
```
...
## 1 runUMAP(tiny_sce_pca, BPPARAM = SerialParam())      6.8s   6.8s        0B
## 2 runUMAP(tiny_sce_pca, BPPARAM = MulticoreParam())  8.31s  8.31s        0B
```
...
other attached packages:
 ...
 [3] BiocNeighbors_1.3.5         scran_1.13.19              
 [5] BiocSingular_1.1.6          bench_1.0.4                
 [7] scater_1.13.18              ggplot2_3.2.1              
 [9] SingleCellExperiment_1.7.9  SummarizedExperiment_1.15.9
