...
This is based on a mathematical technique called the singular decomposition decomposition (SVD).
...
Specifically, PCA is generally robust to noise but an excess of it may cause the earlier PCs to capture noise instead of biological structure.
This effect can be avoided - or at least mitigated - by restricting the PCA to HVGs with large biological components. 
Any of the selection strategies described in Chapter \@ref(feature-selection) could be used for feature selection here.
...
Instead, we obtain a suitable value for $T$ by calculating the proportion of variance in the data that is attributed to the biological component.
This is done using the `denoisePCA()` function with the variance modelling results from `modelGeneVar()` or related functions.
...
```r
# Getting (one of) the closest n.pcs to n.clusters - 1.
n[which.min(abs(collected-1-n))]
```
...
other attached packages:
 [1] scran_1.13.16               BiocSingular_1.1.5         
...
[55] rmarkdown_1.13           gtable_0.3.0            
[57] reshape2_1.4.3           PCAtools_1.1.9          
