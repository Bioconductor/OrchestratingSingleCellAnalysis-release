...
## 1 runPCA(tiny_sce, BSPARAM = IrlbaParam())    222ms    223ms        0B
## 2 runPCA(tiny_sce, BSPARAM = ExactParam())    733ms    733ms        0B
```
...
##   <bch:expr>                                          <bch> <bch:>
## 1 buildSNNGraph(tiny_sce_pca, BNPARAM = AnnoyParam()) 1.57s  1.57s
...
## 1 buildSNNGraph(big_sce, BNPARAM = AnnoyParam())    10.2s   10.2s        0B
## 2 buildSNNGraph(big_sce, BNPARAM = KmknnParam())    15.5s   15.5s        0B
```
...
## 2 runUMAP(tiny_sce_pca, BPPARAM = MulticoreParam())  4.69s  4.69s        0B
```
...
other attached packages:
 ...
 [3] BiocNeighbors_1.3.3         scran_1.13.13              
 [5] BiocSingular_1.1.5          bench_1.0.3                
 [7] scater_1.13.17              ggplot2_3.2.1              
