...
p2 <- plotReducedDim(sce, 'UMAP', colour_by = 'Sample')
wrap_plots(p1, p2, nrow = 1)
```
...
pcs <- batchelor::multiBatchPCA(sce_mnn_advanced, batch = sce_mnn_advanced$Sample,
                                subset.row = hvg_subset,
                                get.variance = TRUE,
                                BSPARAM = BiocSingular::IrlbaParam(),
...
retain <- denoisePCANumber(
    ...
    sum(dec[hvg_subset, ]$tech),       # technical noise in subset of genes.
...
sce_naive <- runPCA(sce_naive, ncomponents = 25,
              feature_set = hvg_subset,
              BSPARAM = IrlbaParam())
...
p1 <- plotReducedDim(sce_limma, 'PCA_limma', colour_by = 'Sample')
p2 <- plotReducedDim(sce_limma, 'UMAP_limma', colour_by = 'Sample')
wrap_plots(p1, p2, nrow = 1)
```
