...
sce <- readRDS("_rfiles/_data/largescale_sce_min.rds")
...
```{r}
set.seed(1234)
subsample <- sample(ncol(sce), 100) # super downsample temporary
sce <- sce[, subsample]
```
...
sub <- counts(sce)[metadata(sce)$genes_keep, ]
clusters <- mbkmeans(sub,
                     max_iters = 1, # reduced for faster clustering
                     clusters = 5,
                     batch_size = 50)

metadata(sce)$prenorm_clusters <- clusters$Clusters
...
sce$clusters_final <- as.factor(clusters_final$Clusters)
```
...
sce_min <- SingleCellExperiment(
    ...
    rowData = rowData(sce)[, c('ID', 'Symbol')],
